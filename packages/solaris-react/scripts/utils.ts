...
const isElementEventMap = (node: Statement): node is TSInterfaceDeclaration =>
  ...
  node.id['name'].endsWith('ElementEventMap');

const isGlobalTSModuleDeclaration = (
  node: Statement
...
const isTSPropertySignature = (
  node: TSTypeElement
): node is TSPropertySignature => node.type === 'TSPropertySignature';
...
const toTypeDeclaration = (
  elementName: string,
  customEvents?: { name: string; events: string[] }
) =>
