...
const WEB_COMPONENTS_DIST_DIR = join(
  __dirname,
  '..',
  '..',
  'solaris-core',
...
const createReactWrapperModules = async ({ entryPoints, distRoot }) => {
  ...
  try {
    ...
    const types: string[] = [];
    const componentsTypePath = join(
      WEB_COMPONENTS_DIST_DIR,
      'types',
      'components.d.ts'
    );
    ...
    for (const importPath of entryPoints) {
      ...
      const tagName = Object.values<Record<string, string>>(component).find(
        (value) => Object.keys(value).includes('is')
      )?.is;
      if (!tagName) {
    ...
    writeFileSync(
      ...
      toTypesFile(types, enums, componentsEvents)
    );
  } catch (err) {
...
createReactWrapperModules({
  entryPoints: readdirSync(COMPONENTS_DIR)
    .filter((file) => file.startsWith('solaris') && file.endsWith('.js'))
